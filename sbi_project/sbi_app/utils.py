...
def process_kalman_cluster_fusion(event_data):
    ...
    try:
        ...
        for i, event in enumerate(event_data):
            event['cluster'] = int(clusters[i])  # Convert numpy.int64 to Python int
        ...
        for c in clusters:
            if c != -1:
                cluster_key = int(c)  # Convert numpy.int64 to Python int
...
def get_cluster_info(event_data):
    ...
    return convert_numpy_types(cluster_info)
...
def calculate_prediction_accuracy(events, prediction):
    ...
    if distances:
        ...
        return convert_numpy_types({'accuracy': accuracy, 'avg_distance': avg_distance})
